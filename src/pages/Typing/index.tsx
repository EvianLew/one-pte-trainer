...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!IsDesktop()) {
      setTimeout(() => {
        alert(
          '壹ONE PTE 词汇训练器目前以桌面键盘练习为主。建议使用电脑浏览器，或在平板设备上连接外接键盘。',
        )
      }, 500)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if (!(id in idDictionaryMap)) {
      setCurrentDictId('pte-fib-listening')
      setCurrentChapter(0)
      return
    }
  ...
  useEffect(() => {
    if (words !== undefined) {
      ...
      dispatch({
        ...
        payload: { words, shouldShuffle: currentDictInfo.mode === 'sgd' ? false : randomConfig.isOpen, initialIndex },
      })
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [words, currentDictInfo.mode, randomConfig.isOpen])
