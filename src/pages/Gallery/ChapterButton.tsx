...
export const ChapterButton: React.FC<ChapterButtonProps> = ({ index, selected, wordCount, onClick, title, subtitle }) => {
  ...
  return (
    <button
      ...
      title="选择章节"
    >
      <p className="w-full pb-2 text-base font-semibold leading-tight text-gray-800 dark:text-white dark:text-opacity-80">{title ?? `Chapter ${index + 1}`}</p>
      {subtitle && <p className="mb-1 text-[11px] font-medium text-indigo-500 dark:text-indigo-300">{subtitle}</p>}
      <p className="text-xs font-medium text-gray-600 dark:text-white dark:text-opacity-60">{title ? '训练短句' : '单词数'}: {wordCount}</p>
      {chapterStatus !== null && (
        <>
